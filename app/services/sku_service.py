import uuid

from fastapi import HTTPException, status
from sqlalchemy import select, func
from sqlalchemy.ext.asyncio import AsyncSession

from app.models.product import Product, ProductStatus, SKU
from app.schemas.product import SKUCreate
from app.services.moderation_client import emit_product_created


async def add_sku(
    db: AsyncSession,
    data: SKUCreate,
    seller_id: uuid.UUID,
) -> SKU:
    # 1. Загружаем товар и проверяем владельца
    product: Product | None = await db.get(Product, data.product_id)
    if product is None or product.seller_id != seller_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")

    # 2. HARD_BLOCKED — добавлять SKU запрещено (канон-флоу: unhappy path)
    if product.status == ProductStatus.HARD_BLOCKED:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Cannot add SKU to a HARD_BLOCKED product",
        )

    # 3. Создаём SKU
    sku = SKU(
        product_id=product.id,
        price=data.price,
        images=data.images,
        attributes=data.attributes or {},
    )
    db.add(sku)

    # 4. Считаем уже существующие SKU (до flush нового)
    existing_count_result = await db.execute(
        select(func.count()).where(SKU.product_id == product.id)
    )
    existing_count = existing_count_result.scalar_one()

    is_first_sku = existing_count == 0

    # 5. Первый SKU — переводим товар в ON_MODERATION
    if is_first_sku:
        product.status = ProductStatus.ON_MODERATION

    await db.commit()
    await db.refresh(sku)

    # 6. Отправляем событие в Moderation только для первого SKU
    if is_first_sku:
        emit_product_created(
            product_id=product.id,
            seller_id=product.seller_id,
            category_id=product.category_id,
            title=product.title,
            images=product.images,
            sku_id=sku.id,
            price=sku.price,
        )

    return sku